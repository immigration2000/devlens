 ...
 */
export function computeCyclomaticComplexity(rootNode) {
    if (!rootNode) {
        return 0;
    }
 ...
 */
export function computeCognitiveComplexity(rootNode) {
    if (!rootNode) {
        return 0;
    }
    let complexity = 0;
 ...
 */
export function computeFunctionMetrics(rootNode) {
    if (!rootNode) {
        return [];
    }
    const functions = [];
    const maxNestingMap = {};
    function traverse(node, nestingDepth = 0) {
 ...
 */
export function computeMaxNestingDepth(rootNode) {
    if (!rootNode) {
        return 0;
    }
    let maxDepth = 0;
 ...
 */
function countFunctionParams(node) {
    if (node.type === "function_declaration") {
