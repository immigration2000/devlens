...
import { computeCyclomaticComplexity, computeCognitiveComplexity, computeFunctionMetrics, computeMaxNestingDepth, } from "../metrics";
/**
 * Create a mock syntax node for testing
 */
function createMockNode(type, text, startRow = 0, startCol = 0, endRow = 0, endCol = 0, children = []) {
    const node = {
        type,
        text,
...
describe("Complexity Metrics", () => {
    it("should compute cyclomatic complexity 1 for simple function with no branches", () => {
        ...
        const complexity = computeCyclomaticComplexity(mockNode);
    ...
    it("should compute cyclomatic complexity for function with if statement", () => {
        ...
        const complexity = computeCyclomaticComplexity(funcNode);
    ...
    it("should increment complexity for multiple if statements", () => {
        ...
        const funcNode = createMockNode("function_declaration", "function foo() { if (x) {} if (y) {} if (z) {} }", 0, 0, 4, 0, [if1, if2, if3]);
        const complexity = computeCyclomaticComplexity(funcNode);
    ...
    it("should count logical operators in complexity", () => {
        ...
        const funcNode = createMockNode("function_declaration", "function foo() { return x && y; }", 0, 0, 1, 0, [binaryExpr]);
        const complexity = computeCyclomaticComplexity(funcNode);
        expect(complexity).toBeGreaterThanOrEqual(2);
    });
    it("should compute cognitive complexity with nesting penalty", () => {
        ...
        const cognitive = computeCognitiveComplexity(funcNode);
    ...
    it("should increase cognitive complexity with nesting", () => {
        ...
        const funcNode = createMockNode("function_declaration", "function foo() { if (x) { if (y) {} } }", 0, 0, 3, 0, [outerIf]);
        const cognitive = computeCognitiveComplexity(funcNode);
        expect(cognitive).toBeGreaterThan(0);
    });
    it("should compute max nesting depth", () => {
        ...
        const funcNode = createMockNode("function_declaration", "function foo() { if (x) { if (y) {} } }", 0, 0, 3, 0, [outerIf]);
        const maxDepth = computeMaxNestingDepth(funcNode);
...
describe("Function Metrics", () => {
    it("should extract function name", () => {
        ...
        const funcNode = createMockNode("function_declaration", "function myFunc() { return 1; }", 0, 0, 1, 0, [nameNode]);
        nameNode.parent = funcNode;
        const metrics = computeFunctionMetrics(funcNode);
    ...
    it("should calculate function line count", () => {
        const funcNode = createMockNode("function_declaration", "function foo() {\n  return 1;\n}", 0, 0, 2, 0);
        const metrics = computeFunctionMetrics(funcNode);
        expect(metrics.length).toBeGreaterThan(0);
        if (metrics.length > 0) {
    ...
    it("should compute cyclomatic complexity per function", () => {
        ...
        const funcNode = createMockNode("function_declaration", "function testFunc() {\n  if (x) {}\n}", 0, 0, 2, 0, [nameNode, ifNode]);
        nameNode.parent = funcNode;
        const metrics = computeFunctionMetrics(funcNode);
        expect(metrics.length).toBeGreaterThan(0);
        if (metrics.length > 0) {
...
describe("Edge Cases", () => {
    it("should handle null nodes gracefully", () => {
        const complexity = computeCyclomaticComplexity(null);
        expect(complexity).toBe(0);
        const cognitive = computeCognitiveComplexity(null);
        expect(cognitive).toBe(0);
        const metrics = computeFunctionMetrics(null);
        expect(metrics).toEqual([]);
        const maxDepth = computeMaxNestingDepth(null);
        expect(maxDepth).toBe(0);
    });
    it("should handle empty trees", () => {
        const emptyNode = createMockNode("program", "");
        const complexity = computeCyclomaticComplexity(emptyNode);
        expect(complexity).toBe(1); // Base complexity
        const cognitive = computeCognitiveComplexity(emptyNode);
        expect(cognitive).toBe(0);
    });
});
